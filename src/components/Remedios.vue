...
const abrirAjudaIA = async (nomeRemedio) => {
  ...
  falar(`Consultando informações sobre ${nomeLimpo}`)
  ...
  try {
    ...
    let texto = data.response?.trim()

    if (!texto) {
      texto = 'Não encontrei informações. Consulte um médico.'
    }
    ...
    const fallback =
      'Não foi possível buscar informações. Consulte um profissional de saúde.'
...
const confirmarTomado = async (remedio) => {
  ...
  falar(`Você tomou ${remedio.nome}`)
}

const excluirRemedio = async (id, nome) => {
  if (confirm(`Remover ${nome}?`)) {
    await deleteDoc(doc(db, 'remedios', id))
    falar('Remédio removido')
  }
}
</script>

<template>
  <main class="w-screen h-screen bg-black text-white flex p-4 overflow-hidden select-none font-sans box-border relative">
        ...
        <div v-for="remedio in remedios" :key="remedio.id" 
          ...
          <div class="h-full w-full bg-black rounded-[45px] border-[4px] border-black p-8 flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="z-10 mt-4 flex justify-between items-end">
              ...
              <button 
                @click="abrirAjudaIA(remedio.nome)"
                @mouseenter="falar(`Precisa de ajuda com o remédio ${remedio.nome}? aperte esse botão vermelho para ouvir uma explicação simples do que é esse remédio`)"
                class="bg-[#FF0000] text-white py-4 px-10 rounded-[25px] font-[1000] text-3xl uppercase shadow-[0_0_20px_rgba(255,0,0,0.4)] active:scale-95 transition-all border-4 border-black mb-2"
              >
                AJUDA
              </button>
            </div>
    ...
    <Transition name="fade">
      <div v-if="mostrarAjuda" class="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 md:p-8 backdrop-blur-md">
        <div class="max-w-4xl w-full max-h-[95vh] bg-[#0a0a0a] border-8 border-[#FF0000] rounded-[60px] p-6 md:p-10 flex flex-col gap-6 shadow-[0_0_100px_rgba(255,0,0,0.3)] overflow-hidden">
          
          <div class="flex items-center gap-6 border-b-4 border-zinc-800 pb-6 shrink-0">
            <span class="text-6xl md:text-8xl" :class="carregandoIA ? 'animate-pulse' : 'animate-bounce'">🤖</span>
            <div class="overflow-hidden">
              <h3 class="text-[#FF0000] font-[1000] text-2xl md:text-3xl uppercase italic leading-none">Aviso de Segurança</h3>
              <h2 class="text-white font-[1000] text-4xl md:text-6xl uppercase tracking-tighter truncate">{{ infoAjuda.nome }}</h2>
            </div>
          </div>

          <div class="bg-zinc-900 p-6 md:p-8 rounded-[40px] border-2 border-zinc-800 text-center overflow-y-auto scroll-personalizado">
            <p class="text-white font-black text-xl md:text-3xl leading-snug uppercase italic">
              "{{ infoAjuda.texto }}"
            </p>
          </div>

          <button @click="fecharAjuda" class="w-full bg-[#ffff00] text-black py-6 md:py-10 rounded-[40px] font-[1000] text-3xl md:text-5xl uppercase border-8 border-black shadow-[10px_10px_0_0_#fff] shrink-0 active:scale-95 transition-all">FECHAR AJUDA</button>
        </div>
      </div>
    </Transition>
